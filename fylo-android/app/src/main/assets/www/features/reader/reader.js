...
import { BookmarkStorage, FileStorage } from '../../core/storage.js';
...
async function _BookmarkSave(store, data) {
  await BookmarkStorage.save(data.docId, new Set(data.pages ?? []));
}
...
function showReaderMenu(){
  ...
  $('#menu-delete').addEventListener('click',()=>{closeModal();const fid=file.id;if(_rs.docId===fid)destroyReader();Actions.removeFile(fid); FileStorage.remove(fid).catch(()=>{}); bus.emit(EVENTS.FILES_DELETED, { fileId: fid });Router.go(PAGES.FILES);});
}
